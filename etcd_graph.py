...
def get_experiment(num_clients, rw):
    ...
    for i in range(num_clients):
        file_name = "etcd_results/" + str(rw) + "/latency_" + str(i) + "_" + str(num_clients) + ".txt"
        file = open(file_name, "r")
        latency = []
        for l in file.readlines():
            if "Average" not in l and "Throughput" not in l:
                val = int(l)
                latency.append(val)
...
# plt.show()
plt.savefig('etcd_latency_throughput.pdf')
plt.savefig("etcd_latency_throughput.png")